...
import { getChatWithMessages } from "@/db";

import Link from "next/link";
import Transcript from "@/components/transcript";
import { ChatWithMesseges } from "@/types";
import { Separator } from "@/components/ui/separator";

export default async function PreviousChPats() {
  const session = await getServerSession();
  let chats: ChatWithMesseges[] = [];

  if (session?.user?.email) {
    chats = await getChatWithMessages(session?.user?.email);
  }
